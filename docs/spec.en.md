...
via `model` and §6.3.1 finds an OAuth record, the plugin
...
| `openai` (Codex) | `POST https://auth.openai.com/oauth/token` | `{"grant_type":"refresh_token","refresh_token":<refresh>,"client_id":"app_EMoamEEZ73f0CkXaXp7hrann"}` (form body) | `info.access` | `Authorization: Bearer <access>`; `ChatGPT-Account-Id: <info.accountId>`; `OpenAI-Beta: responses=experimental`; `originator: codex_cli_rs`; `accept: text/event-stream`; URL rewrite from `api.openai.com/v1/chat/completions` and `api.openai.com/v1/responses` to `https://chatgpt.com/backend-api/codex/responses`; body normalization from AI SDK `messages` / Responses `input` shorthand to Codex typed `input` items, moving system/developer text to `instructions`; force `store:false`, `stream:true`, and SSE-to-JSON conversion for non-streaming AI SDK calls. |
...
6. For Codex, forces `stream:true` and converts the returned SSE stream back
   to a JSON `Response` when the original AI SDK call was non-streaming
   (`generateText`).
7. Delegates to global `fetch` and returns its `Response`.
