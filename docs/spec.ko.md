...
- 도구 이름 및 도구 출력의 번역 없음. **단, 내장 `question` 도구는 예외**:
  ...
  도구 출력 문자열은 `tool.execute.after`에서 결정론적으로 영어로 복원된다
...
| `openai` (Codex) | `POST https://auth.openai.com/oauth/token` | `{"grant_type":"refresh_token","refresh_token":<refresh>,"client_id":"app_EMoamEEZ73f0CkXaXp7hrann"}` (form 본문) | `info.access` | `Authorization: Bearer <access>`; `ChatGPT-Account-Id: <info.accountId>`; URL을 `api.openai.com/v1/chat/completions` 및 `api.openai.com/v1/responses`에서 `https://chatgpt.com/backend-api/codex/responses`로 재작성; AI SDK `messages` / Responses `input` 축약 본문을 Codex typed `input` 항목으로 정규화하고 system/developer 텍스트를 `instructions`로 이동. |
...
5. OpenAI OAuth 요청 본문을 Codex용으로 정규화한다: `system` / `developer`
   ...
   필드는 안전한 기본값을 받는다.
6. 전역 `fetch`에 위임하고 그 `Response`를 반환한다.
...
2. 플러그인의 `tool.execute.before` 핸들러가 `input.tool === "question"`
   을 필터링하고, 세션에 활성 번역 상태가 있고
   `translate_display_lang !== LLM_LANGUAGE` 인 경우:
   - 영어 args의 깊은 복사본을 스냅샷으로 저장한다.
   - `question`, `header`, 그리고 각 옵션의 `label`, `description`을
     `displayLanguage`로 **병렬** 번역한다. 채팅 텍스트와 동일한
     번역기 인스턴스(`src/translator.ts`)를 사용하므로 플레이스홀더
      보호, 재시도/백오프, 180초 타임아웃이 모두 적용된다.
...
5. 플러그인의 `tool.execute.after` 핸들러가, 해당 도구가 원본 영어
   ...
   - 사용자가 선택한 각 라벨에 대해, 그 인덱스를
     ...
     `snapshot.original[i].options[idx].label`을 반환한다. 번역된 라벨과
     일치하지 않는 자유 입력(custom) 답변은 그대로 통과시킨다.
