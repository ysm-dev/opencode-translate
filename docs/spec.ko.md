...
- 설정된 `trigger` 토큰(기본값 `["$en"]`)이 공백으로 구분된 토큰으로
...
- 트리거 매칭은 활성 번역 상태가 없을 때만, 그리고 그런 사용자 작성 텍스트
  파트 전역에서만 실행된다.
- 각 파트에 대해 순서대로, 그리고 각 키워드에 대해 `trigger` 순서대로,
...
1. **SDK를 통한 opencode 저장 인증.** `client.provider.list()`를 호출하여
   ...
   - 그렇지 않고 `p.key`가 `undefined`이고 `p.env.length > 1`이면 provider는
     다중 변수다: §6.3.3을 따른다.
2. **`@ai-sdk/*` 패키지 기본값.** 위의 어느 것도 해결되지 않으면 팩토리에
   ...
   수단으로 보존한다.
3. **오류.** 2단계조차 자격 증명 누락으로 호출 시점에 throw하는 팩토리를
...
하나가 선택되고 해결이 §6.3.1 1단계의 "OAuth 재사용 활성화" 분기에 도달하면,
...
2. `$XDG_DATA_HOME/opencode/auth.json` — `xdg-basedir` 의미로 해결한다.
   `XDG_DATA_HOME`이 없으면 `~/.local/share/opencode/auth.json`으로 폴백한다.
3. 둘 다 존재하지 않거나 JSON 파싱에 실패하면 OAuth
   재사용은 `undefined`를 반환하고 해결기는 §6.3.1의 2단계로 폴백한다.
...
사용자는 `model`을 API 키 provider로 변경하거나 provider의 정본 환경 변수를
구성할 수 있다.
...
| 해결기가 번역기 provider에 대해 사용 가능한 자격 증명을 찾지 못함 | `[opencode-translate:AUTH_UNAVAILABLE] No credential found for provider "{providerID}". Set {envVar} in the environment or run "opencode auth login {providerID}".` |
...
| `trigger` | string[] | `["$en"]` | 비활성 루트 세션의 임의 사용자 메시지에 존재할 때 그 메시지부터 번역 모드를 활성화하는 토큰. 공백 구분 토큰으로 매칭; 대소문자 구분. |
...
- **`auth.test.ts`**
  - 우선순위 순서: OpenAI OAuth가 해결된 provider 키를 이긴다.
...
- **opencode 인증 공유를 v1로 승격(§6.3).** v4는 인증 공유를 v1 범위 외로
  ...
  리프레시 + 커스텀 fetch 흐름을 재구성한다. 우선순위 순서: opencode 저장
  인증 → 환경 변수(ai-sdk 기본 폴백). OAuth 리프레시는
...
20. 구성 옵션: `model`, `trigger`, `sourceLanguage`,
...
25. opencode 인증과의 자격 증명 공유: v1에서 번역기는 opencode의 저장된 인증과
    ...
    `github-copilot`에 대해 항상 켜져 있다. 우선순위 순서는 opencode 저장 인증
    → 환경 변수. 구성 토글 없음; Anthropic OAuth에 대한
...
- 인증 / 자격 증명 해결(모두 §6.3에서 참조됨):
  ...
  - `packages/opencode/src/auth/index.ts:9` — `auth.json` 파일 경로
    (`path.join(Global.Path.data, "auth.json")`; 쓰기는 0o600 모드 사용).
